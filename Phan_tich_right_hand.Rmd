...
```
```{r}
List_var <- c("M_right", "Kv100_right", "δ_right", "T_right", "A365_right", "A460_right", "Anadn_right", "POM_right", "Ae_right", "An_right", "Am_right", "Ar_right", "Ac_right", "F_Ae_right", "F_An_right", "F_Am_right", "F_Ar_right", "F_Ac_right")
```

## Tạo bảng và Forest plot với biến M_right
```{r}
generate_final_table(d, "M_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "M_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_M_right <- generate_final_table(d, "M_right", subgroup_list)
draw_forest_plot(Forest_M_right)
```
...
```{r}
generate_final_table(d, "Kv100_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "Kv100_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_Kv100_right <- generate_final_table(d, "Kv100_right", subgroup_list)
draw_forest_plot(Forest_Kv100_right)
```


## Tạo bảng và Forest plot với biến δ_right
```{r}
generate_final_table(d, "δ_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "δ_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_δ_right <- generate_final_table(d, "δ_right", subgroup_list)
draw_forest_plot(Forest_δ_right)
```

## Tạo bảng và Forest plot với biến T_right
```{r}
generate_final_table(d, "T_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "T_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_T_right <- generate_final_table(d, "T_right", subgroup_list)
draw_forest_plot(Forest_T_right)
```

## Tạo bảng và Forest plot với biến A365_right
```{r}
generate_final_table(d, "A365_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "A365_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_A365_right <- generate_final_table(d, "A365_right", subgroup_list)
draw_forest_plot(Forest_A365_right)
```

## Tạo bảng và Forest plot với biến A460_right
```{r}
generate_final_table(d, "A460_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "A460_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_A460_right <- generate_final_table(d, "A460_right", subgroup_list)
draw_forest_plot(Forest_A460_right)
```

## Tạo bảng và Forest plot với biến Anadn_right
```{r}
generate_final_table(d, "Anadn_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "Anadn_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_Anadn_right <- generate_final_table(d, "Anadn_right", subgroup_list)
draw_forest_plot(Forest_Anadn_right)
```

## Tạo bảng và Forest plot với biến POM_right
```{r}
generate_final_table(d, "POM_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "POM_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_POM_right <- generate_final_table(d, "POM_right", subgroup_list)
draw_forest_plot(Forest_POM_right)
```

## Tạo bảng và Forest plot với biến Ae_right
```{r}
generate_final_table(d, "Ae_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "Ae_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_Ae_right <- generate_final_table(d, "Ae_right", subgroup_list)
draw_forest_plot(Forest_Ae_right)
```

## Tạo bảng và Forest plot với biến An_right
```{r}
generate_final_table(d, "An_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "An_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_An_right <- generate_final_table(d, "An_right", subgroup_list)
draw_forest_plot(Forest_An_right)
```

## Tạo bảng và Forest plot với biến Am_right
```{r}
generate_final_table(d, "Am_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "Am_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_Am_right <- generate_final_table(d, "Am_right", subgroup_list)
draw_forest_plot(Forest_Am_right)
```

## Tạo bảng và Forest plot với biến Ar_right
```{r}
generate_final_table(d, "Ar_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "Ar_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_Ar_right <- generate_final_table(d, "Ar_right", subgroup_list)
draw_forest_plot(Forest_Ar_right)
```

## Tạo bảng và Forest plot với biến Ac_right
```{r}
generate_final_table(d, "Ac_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "Ac_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_Ac_right <- generate_final_table(d, "Ac_right", subgroup_list)
draw_forest_plot(Forest_Ac_right)
```

## Tạo bảng và Forest plot với biến F_Ae_right
```{r}
generate_final_table(d, "F_Ae_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "F_Ae_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_F_Ae_right <- generate_final_table(d, "F_Ae_right", subgroup_list)
draw_forest_plot(Forest_F_Ae_right)
```

## Tạo bảng và Forest plot với biến F_An_right
```{r}
generate_final_table(d, "F_An_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "F_An_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_F_An_right <- generate_final_table(d, "F_An_right", subgroup_list)
draw_forest_plot(Forest_F_An_right)
```

## Tạo bảng và Forest plot với biến F_Am_right
```{r}
generate_final_table(d, "F_Am_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "F_Am_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_F_Am_right <- generate_final_table(d, "F_Am_right", subgroup_list)
draw_forest_plot(Forest_F_Am_right)
```

## Tạo bảng và Forest plot với biến F_Ar_right
```{r}
generate_final_table(d, "F_Ar_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "F_Ar_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_F_Ar_right <- generate_final_table(d, "F_Ar_right", subgroup_list)
draw_forest_plot(Forest_F_Ar_right)
```

## Tạo bảng và Forest plot với biến F_Ac_right
```{r}
generate_final_table(d, "F_Ac_right", subgroup_list) %>% knitr::kable()
```
Wilcox pairwise test cho các sub group có >2 level
```{r}
perform_pairwise_wilcox(d, "F_Ac_right", subgroup_list) %>% knitr::kable()
```
Forest plot 
```{r}
Forest_F_Ac_right <- generate_final_table(d, "F_Ac_right", subgroup_list)
draw_forest_plot(Forest_F_Ac_right)
```
